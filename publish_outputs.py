#!/usr/bin/env python3
"""Publish exactly the CFW-compatible nodes that passed health checking."""
...
for i,u in enumerate(raw,1):
    ...
    key='|'.join(str(n.get(k,'')) for k in ('type','server','port','uuid','password','cipher'))
...
for n in nodes:
    try:
        # converter.parse() has already validated required fields. Do not impose a
        # second, narrower cipher allow-list: Mihomo/Clash supports more SS methods
        # than the previous hard-coded list and that list was rejecting every healthy
        # SS node in this run.
        p=converter.clash(n)
        if p and p.get('type') in CFW_TYPES: proxies.append(p)
        else: rejected.append({'uri':next((u for u,nn in zip(uris,nodes) if nn is n),''),'reason':'cfw_conversion_rejected'})
    except Exception as e:
        rejected.append({'uri':next((u for u,nn in zip(uris,nodes) if nn is n),''),'reason':'cfw_conversion_error:'+type(e).__name__})
...
text='\n'.join(uris)+'\n' if uris else ''
for fn in ('nodes.txt','shadowrocket.txt','all-nodes.txt'): (OUT/fn).write_text(text,encoding='utf-8')
...
# Zero healthy nodes is a valid outcome; only fail if health-checked nodes existed
# but the converter could not turn any of them into a CFW proxy.
if nodes and not proxies: raise SystemExit('Health-checked CFW nodes could not be converted; refusing to publish an empty CFW profile.')

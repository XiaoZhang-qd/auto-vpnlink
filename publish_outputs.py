...
All syntactically valid nodes remain in the raw protocol feeds. Clash output
is additionally filtered for Mihomo compatibility so one legacy/unsupported
proxy method cannot make the whole YAML fail to load.
"""
...
# Mihomo/Clash Verge Rev accepts these modern Shadowsocks AEAD methods.
# Keep legacy methods in ss.txt/all-nodes.txt, but never put them in clash.yaml.
CLASH_SS_CIPHERS=frozenset({
...
def parse(u,i):
    try:
        ...
        if s=='ssr':
            ...
            return {'type':'ssr','name':name,'server':a[0],'port':int(a[1]),'protocol':a[2],'cipher':a[3],'obfs':a[4],'password':b64d(a[5]),'protocol-param':pp.get('protoparam',''),'obfs-param':pp.get('obfsparam','')}
        if s=='vmess':
...
def clash(n):
    t=n['type']
    # This is the exact failure seen in Clash Verge: chacha20-poly1305 is
    # legacy and is not a valid Mihomo Shadowsocks method.
    if t=='ss' and n.get('cipher','').lower() not in CLASH_SS_CIPHERS:
        return None
...
proxies=[];clash_filtered=0
for n in nodes:
    if n['type'] not in ('ss','ssr','vless','vmess','trojan','hysteria2','tuic'):continue
    p=clash(n)
    if p is None:
        clash_filtered+=1
        continue
...
(OUT/'all-nodes.txt').write_text(all_text,encoding='utf-8')
(OUT/'nodes.txt').write_text(all_text,encoding='utf-8')
...
summary={'discovered':len(raw),'healthy_first':len(healthy_keys),'published_unique':len(ordered),'clash_proxies':len(proxies),'clash_filtered_unsupported':clash_filtered,'protocols':{k:len(v) for k,v in by.items()}}
